...
let update m a =
  ...
  | Action_other.Delete_file.Ask { file } ->
    ...
    let modal = Modal.set_confirm_delete msg m.modal
                ...
                |> Modal.set_fun_bt_ok (fun e -> Action.Delete_file (Action_other.Delete_file.Start { area_id; area_subdirs; filename }))
    in
  ...
  | Action_other.Delete_file.Start { area_id; area_subdirs; filename } ->
    ...
    return m ~c
  | Action_other.Delete_file.Do { area_id; area_subdirs; toast_id; filename } ->
    ...
    let c = [Api.http_delete ~url ~payload (fun status _ -> Action.Delete_file (Action_other.Delete_file.Done { area_id; area_subdirs; toast_id; filename; status }))] in
    return m ~c
  | Action_other.Delete_file.Done { area_id; area_subdirs; toast_id; filename; status } ->
    let m =
      match status with
      | 200 ->
        Js_toast.set_status_ok ~doc:Dom_html.document ~id:toast_id ~delay:5.0 ~msg:("File " ^ filename ^ " deleted");
        { m with area_content = Com.Area_content.(remove_file ~id:area_id ~subdirs:area_subdirs ~filename:filename m.area_content) }
      | _ ->
