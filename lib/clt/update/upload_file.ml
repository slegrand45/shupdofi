...
let update m a =
  ...
  | Action_other.Upload_file.Do { area_id; area_subdirs; toast_id; file } -> (
      ...
      let c = [Api.http_post_file ~url ~file (fun status json -> Action.Upload_file (Action_other.Upload_file.Done { toast_id; status; json; filename }))] in
      return m ~c
    )
  | Action_other.Upload_file.Done { toast_id; status; json; filename } -> (
      let m =
        match status with
        | 201 ->
