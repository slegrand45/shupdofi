...
let update m a =
  ...
  | Action_other.Delete_directory.Ask { directory } ->
    ...
    let modal = Modal.set_confirm_delete msg m.modal
                ...
                |> Modal.set_fun_bt_ok (fun e -> Action.Delete_directory (Action_other.Delete_directory.Start { area_id; area_subdirs; dirname }))
    in
  ...
  | Action_other.Delete_directory.Start { area_id; area_subdirs; dirname } ->
    ...
    return m ~c
  | Action_other.Delete_directory.Do { area_id; area_subdirs; toast_id; dirname } ->
    let () = Js_toast.show ~document ~toast_id in
    let payload = Msg_to_srv.Delete_directory.make ~area_id ~subdirs:area_subdirs ~dirname
    ...
    let c = [Api.http_delete ~url ~payload (fun status _ -> Action.Delete_directory (Action_other.Delete_directory.Done { area_id; area_subdirs; toast_id; dirname; status }))] in
    return m ~c
  | Action_other.Delete_directory.Done { area_id; area_subdirs; toast_id; dirname; status } ->
    let m =
      match status with
      | 200 ->
